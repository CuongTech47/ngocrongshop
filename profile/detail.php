...
<?php 
$check = mysql_result(mysql_query("SELECT COUNT(*) FROM `DLC_Dichvu` WHERE `id`='$id'"), 0);
$checkk = mysql_result(mysql_query("SELECT COUNT(*) FROM `DLC_Dichvu` WHERE `username` = '".$tom['username']."' AND `id`='$id'"), 0);
if($check < 1){
...
$TOM_result = mysql_query("SELECT * FROM `DLC_Dichvu` WHERE `username` = '".$tom['username']."' AND id = '$id' LIMIT 1");
...
<?php if ($getdlc[dichvu] == 1) { ?>
                    <p><a href="/dich-vu/ban-vang"><?php echo $getdichvu[$getdlc[dichvu]] ;?></a></p>
<?php } ?>
