	...
	<div class="row">
		...
		<div class="col-md-12 col-sm-6 col-xs-6 m-t-15">
            ...
            <div class="c-content-title-1">
                <h3 class="c-font-uppercase c-font-bold">Dịch vụ đã thanh toán</h3>
                    ...
                    <div class="col-md-4">
                        <div class="input-group m-b-10 c-square">
                            ...
                            <select id="group_id" name="dichvu" class="form-control c-square c-theme">
                                ...
                                <option value="1"> Bán Vàng (Ngọc rồng)</option>
                                <option value="2"> Bán Ngọc (Ngọc rồng)</option>
                            </select>
            ...
            <table class="table table-hover table-custom-res">
                ...
                <tbody>
<?php
$getdichvu = array("Không xác định", "Bán vàng", "Bán ngọc");
...
$tong = mysql_result(mysql_query("SELECT COUNT(*) FROM `DLC_Dichvu` WHERE ".$locdichvu."  AND ".$loctt." AND `username` = '".$tom['username']."'"), 0);
$TOM_result = mysql_query("SELECT * FROM `DLC_Dichvu` WHERE ".$locdichvu." AND ".$loctt." AND `username` = '".$tom['username']."' ORDER by ID DESC LIMIT $start, $kmess");
} else {
$tong = mysql_result(mysql_query("SELECT COUNT(*) FROM `DLC_Dichvu` WHERE `username` = '".$tom['username']."'"), 0);
$TOM_result = mysql_query("SELECT * FROM `DLC_Dichvu` WHERE `username` = '".$tom['username']."' ORDER by id DESC LIMIT $start, $kmess");
if ($tong == 0) {
?>
...
<?php if ($getdlc[dichvu] == 1) { ?>
                                <td><a  href="/dich-vu/ban-vang" target="_blank"><?php echo $getdichvu[$getdlc[dichvu]]; ?></a></td>
<?php } ?>
<?php if ($getdlc[dichvu] == 2) { ?>
                                <td><a  href="/dich-vu/ban-ngoc" target="_blank"><?php echo $getdichvu[$getdlc[dichvu]]; ?></a></td>
<?php } ?>
                                <td><span class="c-font-bold text-info"><?php echo number_format($getdlc[trigia]); ?>đ</span></td>
