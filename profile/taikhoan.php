...
ob_start();
session_start();
$title = 'Tài khoản ngọc rồng rút vàng';
		...
		<div class="col-md-12 col-sm-6 col-xs-6 m-t-15">
			...
			<div class="c-content-ver-nav m-b-20">
                ...
                <table class="table table-hover table-custom-res">
                    ...
                    <tbody>
<?php
$getnganhang = array("Không xác định", "TKCR(tkcr.vn)", "TCSR(thecaosieure.com)", "TSR(thesieure.com)", "AZPRO(azpro.vn)", "Momo", "Techcombank", "Vietcombank", "Vietinbank", "Bidv", "Mbbank", "Sacombank", "Agribank");
$tong = mysql_result(mysql_query("SELECT COUNT(*) FROM `Hplayer_taikhoan` WHERE `username` = '".$tom['username']."'"), 0);
$TOM_result = mysql_query("SELECT * FROM `Hplayer_taikhoan` WHERE `username` = '".$tom['username']."' ORDER by id DESC LIMIT $start, $kmess");
if ($tong == 0) {
?>
...
<?php if($_GET['id']){
$id = intval($_GET['id']);
$check = mysql_result(mysql_query("SELECT COUNT(*) FROM `Hplayer_rutvang` WHERE `username` = '".$tom['username']."' AND `id`='$id'"), 0);
if($check < 1){
header('Location: /user/bank');
	exit;
} else {
	mysql_query("DELETE FROM `Hplayer_rutvang` WHERE `id` = '$id'");
	header('Location: /user/bank');
}
}
	?>
