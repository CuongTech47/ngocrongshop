...
<tbody>
<?php
$tong = mysql_result(mysql_query("SELECT COUNT(*) FROM `DLC_mualuotquay`"), 0);
$TOM_result = mysql_query("SELECT * FROM `DLC_mualuotquay` ORDER by ID DESC LIMIT $start, $kmess");
while($gettom = mysql_fetch_assoc($TOM_result)){
?>
<tr>
   <th scope="row">#<?php echo $gettom[ID]; ?></th>
   <td><?php echo strip_tags($gettom[nguoimua]); ?></td>
   <td><?php echo $gettom[solan]; ?></td>
...
<?php if ($tong > $kmess){
echo '<center>' . phantrang_tomdz('mualuotmoqua.php?', $start, $tong, $kmess) . '</center>';
} ?>
</div>
</div>
</div>
