...
mysql_query("UPDATE Hplayer_rut-vang SET `tinhtrang` = '".$tinhtrang."', `thoigianduyet` = '".time()."' WHERE `id` = '".intval($_GET['set'])."'");  ?>
...
mysql_query("DELETE FROM Hplayer_rut-vang WHERE id = '".intval($_GET[xoa])."'");
...
$tong = mysql_result(mysql_query("SELECT COUNT(*) FROM `Hplayer_rut-vang`"), 0);
$logtom = mysql_query("SELECT * FROM `Hplayer_rut-vang` ORDER BY id DESC LIMIT $start, $kmess");
